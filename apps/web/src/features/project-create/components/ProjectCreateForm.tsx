...
interface ProjectCreateFormProps {
  ...
  onSubmit: (values: ProjectCreateFormValues) => Promise<{ ok: boolean; projectId?: string; message?: string }>;
}
...
export function ProjectCreateForm({
  ...
  const selectedTemplate = useMemo(
    () => templates.find((template) => template.id === values.templateId) ?? templates[0] ?? null,
    ...
    <div className="min-h-screen bg-gray-100">
      <header className="border-b border-gray-200 bg-white">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-4 px-6 py-4">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">按模板快速生成项目卡片、部门工作流和默认协同结构。</p>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto grid max-w-5xl gap-4 px-6 py-6 lg:grid-cols-[minmax(0,2fr)_minmax(320px,1fr)]">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Field label="优先级">
              <select
                ...
                onChange={(event) => handleFieldChange("priority", event.target.value as ProjectCreateFormValues["priority"])}
                className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-700"
              >
            ...
            <Field label="SLA 截止时间">
              <input
                ...
                onChange={(event) => handleFieldChange("slaDeadline", event.target.value)}
                className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-700"
              />
            </Field>
          </div>
        ...
        <aside className="space-y-4">
            ...
            {selectedTemplate ? (
              <>
                ...
                <div className="mt-3 flex items-center gap-2 text-sm text-gray-500">
                  ...
                  <span>·</span>
                  <span>默认优先级 {PRIORITY_LABELS[selectedTemplate.defaultPriority]}</span>
                </div>
              </>
            ) : (
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            <h2 className="text-base font-semibold text-gray-900">默认部门工作流</h2>
            <div className="mt-3 space-y-2">
              {selectedTemplate?.departments.map((department) => (
                <div key={department.departmentId} className="rounded-lg bg-gray-50 px-3 py-2 text-sm text-gray-700">
                  {department.departmentName}
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            <h2 className="text-base font-semibold text-gray-900">默认审批链</h2>
            <div className="mt-3 space-y-2">
              {selectedTemplate?.approvalGates.length ? (
                selectedTemplate.approvalGates.map((gate) => (
                  <div key={`${gate.approvalCode}-${gate.triggerStage}`} className="rounded-lg bg-gray-50 px-3 py-2 text-sm text-gray-700">
                    {gate.title}
                    <p className="mt-1 text-xs text-gray-400">触发阶段：{gate.triggerStage}</p>
                  </div>
                ))
              ) : (
