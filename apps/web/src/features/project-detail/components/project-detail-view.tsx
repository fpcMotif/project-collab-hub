...
const DepartmentTrackRow = ({
  ...
  <div className="rounded-lg border border-gray-200 p-3">
    ...
    {track.blockReason && (
      <p className="mt-2 text-xs text-red-600">阻塞原因：{track.blockReason}</p>
    )}
  </div>
);
...
const TimelineRow = ({ event }: { event: ProjectDetailTimelineEvent }) => (
  <div className="rounded-lg border border-gray-200 p-3">
    <div className="flex items-center justify-between gap-2 text-xs text-gray-500">
...
const BindingGroup = ({ title, items }: { title: string; items: string[] }) => (
...
export const ProjectDetailNotFound = ({ projectId }: { projectId: string }) => (
