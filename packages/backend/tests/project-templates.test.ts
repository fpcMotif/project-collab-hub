...
describe("projectTemplates", () => {
  ...
  it("getById query works", async () => {
    ...
    await t.run(async (ctx) => {
        const tempId = await ctx.db.insert("projectTemplates", {
            approvalGates: [],
            chatPolicy: { addBotAsManager: true, autoCreateChat: true, pinProjectCard: true },
            createdBy: "user_1",
            defaultPriority: "medium",
            departments: [],
            description: "temp",
  ...
  it("createNewVersion mutation works", async () => {
    ...
    const newTemplateId = await asUser.mutation(api.projectTemplates.createNewVersion, {
      sourceTemplateId,
    });
  ...
  it("createNewVersion throws if not authenticated", async () => {
    ...
    await expect(
      t.mutation(api.projectTemplates.createNewVersion, {
        sourceTemplateId: "some-id" as any,
      })
